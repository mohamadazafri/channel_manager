...
class ApiCredentialService {
  ...
  ApiCredentialService({FlutterSecureStorage? storage}) : _storage = storage ?? const FlutterSecureStorage();
  ...
  Future<void> saveCredentials(String platform, Map<String, String> credentials) async {
    await _storage.write(key: '${platform.toLowerCase()}_credentials', value: jsonEncode(credentials));
  }
  ...
  Future<List<String>> listPlatformsWithCredentials() async {
    ...
    return allKeys.keys.where((key) => key.endsWith('_credentials')).map((key) => key.replaceAll('_credentials', '')).toList();
  }
}
